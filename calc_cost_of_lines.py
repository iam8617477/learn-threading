...
# Глобальная переменная для хранения частот букв
count = Counter()

def filter_sentence(sentence):
    sentence = sentence.lower()
    return ''.join(re.findall(r'[^\W\d_]', sentence))
...
# Список messages=[] располагается "под капотом" к нему можно обратиться и получить весь список предложений.
...
print(dict(sorted(count.items(), key=lambda item: item[0])))
